...
# charger les données
df <- read.dta("Propre/cirano_leger_covid_7.dta")
df <- df[df$semaine==7,]
...
#nks <- c(25254,181561,473358)
nks <- c(25254,181561,475675) # mise à jour (+précision) 9 mars
nks <- c(nks,sum(nks))
nks
...
tableau <- t(tableau)
print(xtable(tableau,digits=0),file='Tableaux-Figures/covid-prevalence-vague7.tex')
round(tableau)
...
ggplot(fig) +
  ...
  theme_bw()
ggsave('Tableaux-Figures/prevalence-covid-vague7.png',dpi=1200)

saveRDS(fig,file="Tableaux-Figures/tableauV7.Rda")
