...
# charger les données
df <- read.dta("Propre/cirano_leger_covid_9.dta")
df <- df[df$semaine==9,]
...
tableau <- t(tableau)
print(xtable(tableau,digits=0),file='Tableaux-Figures/covid-prevalence-vague9.tex')
round(tableau)
...
ggplot(fig) +
  ...
  theme_bw()
ggsave('Tableaux-Figures/prevalence-covid-vague9.png',dpi=1200)

saveRDS(fig,file="Tableaux-Figures/tableauV9.Rda")
