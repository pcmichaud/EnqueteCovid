...
# charger les données
df <- read.dta("Propre/cirano_leger_covid_4.dta")
df <- df[df$semaine==4,]
...
#nks <- c(25246,181561,484257)
nks <- c(25246,181561,483984)  # mise à jour (+précision) 9 mars
nks <- c(nks,sum(nks))
nks
...
tableau <- t(tableau)
print(xtable(tableau,digits=0),file='Tableaux-Figures/covid-prevalence-vague4.tex')
round(tableau)
...
ggplot(fig) +
  ...
  theme_bw()
ggsave('Tableaux-Figures/prevalence-covid-vague4.png',dpi=1200)

saveRDS(fig,file="Tableaux-Figures/tableauV4.Rda")
